...
let trans_tok_pat_8cc7dbf ((kind, body) : mt) : CST.tok_pat_8cc7dbf =
  match body with
  | Leaf v -> v
  | Children _ -> assert false
...
let trans_identifier ((kind, body) : mt) : CST.identifier =
  ...
  | Children v ->
      ...
      | Alt (1, v) ->
          `Tok_pat_8cc7dbf (
            trans_tok_pat_8cc7dbf (Run.matcher_token v)
          )
      | _ -> assert false
      )
  | Leaf _ -> assert false
...
and trans_parameter ((kind, body) : mt) : CST.parameter =
  ...
  | Children v ->
      (match v with
      | Alt (0, v) ->
          `Rep_attr_list_opt_param_modi_opt_type_id_opt_equals_value_clause (
            (match v with
            | Seq [v0; v1; v2; v3; v4] ->
                (
                  Run.repeat
...
and trans_record_base_ ((kind, body) : mt) : CST.record_base_ =
  match body with
  | Children v ->
      (match v with
      | Alt (0, v) ->
      ...
      | Seq [v0; v1; v2; v3; v4; v5; v6; v7; v8] ->
          (
            ...
            Run.repeat
              ...
              v7
            ,
            trans_record_base_ (Run.matcher_token v8)
          )
      | _ -> assert false
      )
  | Leaf _ -> assert false
